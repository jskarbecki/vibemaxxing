...
| round | 1 |
| status | OPEN |
| note | PARTLY ADDRESSED. 0.2.1: the 60/120/240/480 backoff now governs every live request, per account and across processes, through the shared cache in `usage/<alias>.json` (CONTRACT section 14), and one account costs at most one fetch per ~3 min however many surfaces are open. Still unused: `Scheduler` itself, its stagger and MIN_GAP_S, and `record_exhausted`. The 0.2.0 web cache also let two tabs whose timers aligned both fetch (measured: paired samples 10 ms apart in history.db), fixed in 0.2.1. Earlier: the rate half is fixed: both long-lived surfaces now poll at `poll.DASHBOARD_INTERVAL_S` (180 s, ~20 requests/hour, inside the measured 28-30/hour budget) and the web dashboard has a cache so N open tabs are one upstream fetch, tested. The stagger and the 60/120/240/480 backoff still govern no live request. Wiring `Scheduler` into `refresh_cycle` would gate work behind a clock that AC14 holds fixed across its 200 cycles, hollowing out that measurement; doing it properly needs the refresh restructured into a per-account path, which is a change worth its own milestone rather than one bolted on at v0.1.0. |
